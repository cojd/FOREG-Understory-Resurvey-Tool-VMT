...
$(function () {
  ...
  function watchForm(params)
  {
    ...
    if (f[0].checkValidity() === true) {
      ...
      if ([6, 7, 8, 9].includes(Number(params.status))) params.form_def = 'mortality'; // send to mortality form if status is dead or crushed
...
function queryDB(table, query, params) {
  ...
  var successFn = function (result) {
    ...
    for (var row = 0; row < result.getCount(); row++) {
      ...
      r['id']     = result.getData(row, "ID");
      r['idQual']     = result.getData(row, "PrevIDqual");
      ...
      r['cell']       = result.getData(row, "Cell");
      r['PrevYear']   = result.getData(row, "PrevYear");
      r['species']    = result.getData(row, "Species");
      r['status']     = result.getData(row, "PrevStatus");
      r['ht']     = result.getData(row, "PrevHt");
      r['comments']   = result.getData(row, "PrevComments");
      r['da_dbh'] = result.getData(0, 'AdultDBH');
      r['da_species'] = result.getData(0, 'AdultSp');
      r['empty_cells'] = result.getData(0, 'EmptyCells');
      r['percent_cover'] = result.getData(0, 'PlantCover');
      r['percent_moss'] = result.getData(0, 'MossCover');
      r['da_tag'] = result.getData(0, 'AdultTag');

      // records.push(r);
      // build a string from some of the values
      let st = 'Species: ' + r.species + ' | Tag: ' + r.tag + ' | Status: ' + DataLists.StatusList[r.status];
      records[r.plot] = r;
