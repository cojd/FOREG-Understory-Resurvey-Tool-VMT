...
function loadDataIntoSelects()
{
  ...
  stand.change(function () {
    let plot = $('#plot').html('<option value="">Please select an understory plot</option>');
    // Modify the query to fetch from the table that contains all the plots
    odkData.arbitraryQuery('prev_standdoc', 'SELECT DISTINCT Plot FROM prev_standdoc WHERE StandID=?', [stand.val()], null, null, plotSuccess, console.log);
  });
}

function loadDataIntoSelectsOld()
{
  /////////////////////////////////////////// CALLBACKS
  ///////////////////////////////////////////
  let standSuccess = function (result) {
