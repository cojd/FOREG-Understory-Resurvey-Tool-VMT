$(function () {
  ...
  let success = function(result) {
    ...
    for (var row = 0; row < result.getCount(); row++) {
      ...
      let ID = result.getData(row, "ID");
      let IDqual = result.getData(row, "PrevIDqual");
      ...
      let cell = result.getData(row, "Cell");
      let status = result.getData(row, "PrevStatus");
      let height = result.getData(row, "PrevHt");
      let gx = result.getData(row, "gx");
      let gy = result.getData(row, "gy");


      let item = $('<tr> <th scope="row">' + tag + '</th> <td>' + DataLists.SpeciesList[species] + '</td> <td>' + DataLists.StatusList[status] + '</td> <td>' + height + '</td> <td>' + cell + '</td> </tr>');
  ...
  query += " AND prev_data.PrevStatus NOT IN (6, 7, 8, 9)" // dont display dead trees in prev_data as missing
