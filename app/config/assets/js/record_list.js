...
$(function () {
  ...
  if (search.length) search.change(() => { // this search bar doesn't exist in the stand docs records list, only for measure and mortality
    ...
    {
      let shown = 0;
      // this probably won't perform very well if the list has a lot of records in it. this and pagination or requerying would probably be better
      records.each(function (index) { // go through each item card
        let record = $(this);
        if (record.data('tag') == val) // check if its tag matches and show it
        {
          record.show();
          shown++;
        } else record.hide(); // if it doesn't, hide it
      });
...
var success = function (resultSet) {
  ...
  $('.container').click(function (e) {
    ...
    if (rowId !== null && rowId !== undefined) {
      ...
      // Opens the detail view from the file specified in the properties worksheet
      odkTables.openDetailView(null, tableId, rowId, null);
    } else {
      console.log('RowId is null or undefined');
    }
  });
  ...
  for (var i = 0; i < resultSet.getCount(); i++) {
    ...
    else if (tableId == 'herbs') appendHerbItem(i, resultSet);
...
function appendTreeItem(row, resultSet, tableId) {
  ...
  let ID = resultSet.getData(row, 'ID');
  ...
  Utils.get_tree_prev(stand, plot, tag, function (prev) {
    ...
    <i>${DataLists.SpeciesList[species]}</i>
    </div>
    </div>
    `;
    let item = $(h);
    item.attr('id', resultSet.getRowId(row));
    item.attr('rowId', resultSet.getRowId(row)); // don't think this is used
    item.data('tag', tag);
...
function appendHerbItem(row, resultSet) {
  // Creates the item space and stores the row ID in it
  var herb_species = resultSet.getData(row, 'herb_species');
  var stemcount = resultSet.getData(row, 'stemcount');
  var cover = resultSet.getData(row, 'cover');
  var avgcell = resultSet.getData(row, 'avgcell')
  let h = `
    <div class="card">
      <div class="card-body">
        ${herb_species}
        <div class="form-group">
          <textarea class="form-control" rows="1" readonly disabled>Stem Count: ${stemcount}, Cells Covered: ${cover}, Average Cell: ${avgcell}</textarea>
        </div>
      </div>
    </div>
    `;
