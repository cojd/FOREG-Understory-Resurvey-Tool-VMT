...
function populateFormFromPrev(prev) {
  ...
  $.each(inputs, function () {
    ...
    if (!a) return; // continue if it didn't have prev_action set
...
function buildForm(form, editing) {
  ...
  `);
  // it's important to have the type of the cancel button set to button so it doesn't also submit the form when clicked
  // just about lost my mind for a minute when that was happening. seems to default to submit
}
...
function watchForm(params) {
  ...
  f.submit(function (event) {
    ...
    if (params.form_def && params.form_def == "mortality") delete data.height;
...
function formatFormData(formData) {
  ...
  for (let i = 0; i < formData.length; i++) // serializeArray outputs a 2d array for the form [["key", "value"], ...]
  {
    ...
    if (item.value !== '' && item.value !== null && item.value !== undefined) { // as long as it isn't empty or null
...
function createRow(tableId, data) {
  console.log(`createRow parameters: tableId = ${tableId}, data = `, data);
...
function editRow(tableId, rowId, data) {
  console.log(`editRow parameters: tableId = ${tableId}, rowId = ${rowId}, data = `, data);
